...
//            logger.info("t=" + t);
	...
	public static Planning localSearch(Planning optimizedPlanning) throws IOException {
		changeList.clear();
		/*
		 * Jonas: Eenvoudiger om gewicht toe te voegen aan een localsearch_step.
		 * localsearchUpperBound moet niet aangepast worden, dat doet de code zelf bij
		 * de eerste itteratie
		 * 
		 * Enkel de getalletjes dus verhogen/verlagen als je wilt dat een
		 * localsearch_step meer/minder moet uitgevoerd worden.
		 * 
		 */
		...
		/*
		 * //TODO: add more steps //SHIPPING DAYS VAST LEGGEN (wisselen, verwijderen,
		 * toeveogen) //REQUESTS BEHANDELEN //switch production types
		 * 
		 * 
		 * local search operators: - productie van een item wisselen met productie van
		 * een andere item -> 1 block per keer of alle blocks van een item wisselen -
		 * productie van een item verplaatsen -> 1 block per keer of alle blocks van een
		 * item verplaatsen - ketting van operators -> na elkaar uitvoeren van een
		 * aantal operators (extra) - productie van een item verwisselen in alle machine
		 * - add overdag product (no overtime, no nighshift)
		 */

		// Moet niet fesaible terugeven -> liefst wel!!
	...
	private static void addProduction(Planning p) { // van hetzelfde product meer maken, geen setup nodig voor het item,
													// mss wel na het item
		int count = 0;
		...
		/*
		 * Main.printOutputToConsole(p); System.out.println("iets");
		 */
...
//            Main.printOutputToConsole(p);
	...
	private static boolean setupBeforeNewItem(Item previousItem, Item newItem, int day, int block, Machine machine,
			Planning p, int sp, boolean afterAlreadyChecked) {
		...
		for (int i = 0; i < tijdSetup; i++) {
			if (previousItem.isLargeSetup(newItem)) { // large setup
				p.getDay(day).getBlock(earliestMomentBlock + i).setMachineState(machine,
						new LargeSetup(previousItem, newItem));
			} else { // small setup
				p.getDay(day).getBlock(earliestMomentBlock + i).setMachineState(machine,
						new SmallSetup(previousItem, newItem));
			}
		}
