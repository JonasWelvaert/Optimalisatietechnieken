...
public class FeasibiltyChecker {
    ...
    private boolean checkStockConstraintsExtra(int d, Planning planning) {

        for (Item i : planning.getStock()) {

            int stockAmountPrevDay = 0;
            int productionAmountCurrentDay = 0;
            int shippingAmountCurrentDay = 0;
            int stockAmountCurrentDay = i.getStockAmount(planning.getDay(d));
            int maxAllowed = i.getMaxAllowedInStock();
            int minAllowed = i.getMinAllowedInStock();

            if (d == 0) {
                stockAmountPrevDay = i.getInitialQuantityInStock();
            } else {
                stockAmountPrevDay = i.getStockAmount(planning.getDay(d - 1));
            }

            for (Machine m : planning.getMachines()) {
                for (int b = 0; b < Day.getNumberOfBlocksPerDay(); b++) {
                    if (planning.getDay(d).getBlock(b).getMachineState(m) instanceof Production) {
                        Item itemProduced = ((Production) planning.getDay(d).getBlock(b).getMachineState(m)).getItem();
                        if (itemProduced.getId() == i.getId()) {
                            productionAmountCurrentDay += m.getEfficiency(itemProduced);
                        }
                    }
                }
            }
            for (Request r : planning.getRequests()) {
                if (r.hasShippingDay()) {
                    if (r.getShippingDay().getId() == planning.getDay(d).getId()) {
                        if (r.getItemsKeySet().contains(i)) {
                            shippingAmountCurrentDay += r.getAmountOfItem(i);
                        }
                    }
                }
            }
            if ((stockAmountPrevDay + productionAmountCurrentDay - shippingAmountCurrentDay) < 0) {
                return false;
            }
            if ((stockAmountPrevDay + productionAmountCurrentDay - shippingAmountCurrentDay) > maxAllowed) {
                return false;
            }
            if ((stockAmountPrevDay + productionAmountCurrentDay - shippingAmountCurrentDay) != stockAmountCurrentDay) {
                return false;
            }
        }
